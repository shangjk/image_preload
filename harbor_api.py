    ...
    def get_project_events(self, begin_time, end_time):
        ...
        try:
            if self.version == "v2.0":
                ...
                if status:
                    res_url = self.url + "projects/{}/logs?q=operation%3Dcreate%2Cop_time%3D%5B2022-06-01%2010%3A00%3A00~2022-06-16%2023%3A00%3A00%5D".format(
                        self.project_id, begin[0], begin[1], begin[2], begin[3], begin[4], begin[5], end[0], end[1],
                        end[2], end[3], end[4], end[5])
                    res_events = self.session.get(url=res_url, headers=self.Head, timeout=5)
                else:
            ...
            if res_events.status_code == 200:
                ...
                logger.error(
                    "Exception when calling get project %s events: %s" % (self.project_name, res_events.json()))
                return []
...
def main():
    ...
    if cfg['res']:
        ...
        elasticsearch_password = configs.get("harbor_daemon", "elasticsearch_password")
                    ...
                    try:
                        for host in target_pods:
                            if check_connection(host, 8000):  # 判断是否可连通
                                ...
                                jk_thread_name.add_done_callback(result_result)
    ...
    while True:
        ...
        if len(result) > 0:
            ...
            logger.info("Get target client endpoints: %s" % targets)
            if status:
                images_pool = {}
